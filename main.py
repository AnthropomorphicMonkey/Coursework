# Imports required modules for UI programming and control(no missing module validation required as all modules will be
# included when .exe created)
import datetime
import random
import sqlite3
import sys
...
def create_database():
    # Generic code to open and connect to SQL database. If file is not found a new file is created
    database_name: str = 'database.db'
    conn = sqlite3.connect(database_name)
    c = conn.cursor()
    # SQL code below generated by SQLite browser when creating database within it's UI. Code will create all tables as
    # defined if found to be missing at program startup and establishes all field relationships
    ...
    sql: str = 'CREATE TABLE IF NOT EXISTS "users" ( `id` INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT UNIQUE, ' \
               ...
               '`type` TEXT NOT NULL )'
    c.execute(sql, ())
    # Inserts entries into question types table defining the topic names related to each type id. If there is already
    # an entry in the table for the given id, the topic name is made sure to be correct and changed if needed. To ease
    # development, topic ids and names are stored as tuples in a list which is iterated through when inserting data,
    # meaning new topics can simply be added to the list as they are added to the program and the database will be
    ...
    for topic in data:
        ...
        # If topic already in database, ensures topic name is correct and updates accordingly
...
class Window(QtWidgets.QMainWindow, window.Ui_MainWindow):
    ...
    def __init__(self):
        ...
        super().__init__()
        self.setupUi(self)
        # Declare constants related to page indexes of different sections of the program as references to the integer
        ...
        # Holds the id of the current question number while quiz being done to keep track of position within question
        # list
        self.current_question: int = 0
        # Holds the location of the correct answer during a quiz (set to 1 initially as it will be changed when quiz
        # page loaded)
        ...
        if db_scripts.check_user_exists(self.login_username_input.text().casefold()):
            ...
            if db_scripts.check_password(user_id, self.login_password_input.text()):
                ...
                if db_scripts.get_account_type(user_id) in ['s', 't']:
                    ...
                    # Sets the current logged in user class attribute to the now logged in user
            ...
            # If password incorrect, invalid password error is displayed and password box is cleared
                ...
                self.login_password_input.setText("")
        # If username did not exist, invalid username error is displayed and username box is cleared
        else:
